/**
 * Harbor Plugin: Time - Background Script
 *
 * Provides time-related tools to Harbor.
 */
...
const PLUGIN_ID = 'harbor-plugin-time@local';
const PLUGIN_NAME = 'Harbor Plugin: Time';
...
const TOOLS = [TIME_NOW_DEFINITION, TIME_FORMAT_DEFINITION];
...
function executeTool(
  ...
  switch (toolName) {
    case 'time.now':
      return timeNow(args as TimeNowInput);

    case 'time.format':
      return timeFormat(args as TimeFormatInput);
...
async function sendToHub(message: PluginMessageEnvelope): Promise<void> {
  try {
    ...
    console.error('[TimePlugin] Failed to send message to Hub:', err);
  }
}
...
async function handleToolCall(
  ...
  console.log('[TimePlugin] Tool call:', payload.toolName);
...
async function handlePing(requestId: string): Promise<void> {
  console.log('[TimePlugin] Ping received');
...
browser.runtime.onMessageExternal.addListener(
  (message: unknown, sender: browser.Runtime.MessageSender) => {
    if (sender.id !== HARBOR_HUB_EXTENSION_ID) {
      console.warn('[TimePlugin] Ignoring message from unknown sender:', sender.id);
      return;
    }
    ...
    if (envelope.namespace !== PLUGIN_NAMESPACE) {
      console.warn('[TimePlugin] Ignoring message with wrong namespace:', envelope.namespace);
      return;
    }

    console.log('[TimePlugin] Received message:', envelope.type);
    ...
    switch (envelope.type) {
      ...
      case 'PLUGIN_REGISTER_ACK': {
        ...
        if (ack.success) {
          console.log('[TimePlugin] Registration successful');
        } else {
          console.error('[TimePlugin] Registration failed:', ack.error);
        }
        break;
      }

      default:
        console.warn('[TimePlugin] Unknown message type:', envelope.type);
    }
  }
);
...
async function registerWithHub(): Promise<void> {
  console.log('[TimePlugin] Registering with Harbor Hub...');
  ...
  const registerMessage = createMessage('PLUGIN_REGISTER', {
    plugin: {
      ...
      description: 'Time-related tools for Harbor',
      tools: TOOLS,
    },
  });
  ...
  try {
    ...
    console.log('[TimePlugin] Registration message sent');
  } catch (err) {
    console.error('[TimePlugin] Failed to register with Hub:', err);
    console.log('[TimePlugin] Hub may not be installed. Will retry on Hub startup.');
  }
}
...
console.log('[TimePlugin] Background script initialized');
