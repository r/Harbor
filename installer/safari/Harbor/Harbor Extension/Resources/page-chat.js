...
// src/page-chat.ts
var byocConfig = window.__harborPageChatConfig || {};
...
function initPageChat() {
  ...
  const toggleBtn = container.querySelector("#hpc-toggle");
  const closeBtn = container.querySelector("#hpc-close");
  const panel = container.querySelector(".hpc-panel");
  const connectBtn = container.querySelector("#hpc-connect");
  const setupOverlay = container.querySelector(".hpc-setup");
  const setupError = container.querySelector("#hpc-setup-error");
  const messagesContainer = container.querySelector("#hpc-messages");
  const emptyState = container.querySelector(".hpc-empty");
  const inputArea = container.querySelector("#hpc-input");
  const sendBtn = container.querySelector("#hpc-send");
  const statusDot = container.querySelector("#hpc-status-dot");
  const statusText = container.querySelector("#hpc-status-text");
...
---`;
      ...
      if (response.type === "error" || response.error) {
        ...
        if (response.toolsUsed && response.toolsUsed.length > 0 && messageEl) {
          ...
          toolsInfo.innerHTML = `<small>\u{1F527} Used: ${response.toolsUsed.map((t) => t.name.split("/").pop()).join(", ")}</small>`;
  ...
  function updateMessageBody(messageEl, content) {
...
function getSidebarHTML(isByocMode) {
  const headerContent = isByocMode ? `<div class="hpc-header hpc-header-byoc">
        <div class="hpc-header-main">
          ...
          <div class="hpc-header-right">
            ...
            <button class="hpc-close-btn" id="hpc-close" title="Close">\xD7</button>
          </div>
        </div>
  ...
  return `
    <button class="hpc-toggle" id="hpc-toggle" title="Toggle chat">\u{1F4AC}</button>
    <div class="hpc-panel">
      ${headerContent}
...
//# sourceMappingURL=page-chat.js.map
